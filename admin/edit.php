...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($codeSource === 'upload' && !empty($_FILES['ino_file']['name']) && $_FILES['ino_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($ext === 'ino') {
            $codeContent = trim(file_get_contents($_FILES['ino_file']['tmp_name']));
        }
    }
    
    if (empty($title)) {
        $error = 'Title is required.';
    } elseif (empty($codeContent)) {
        $error = 'Code is required. Paste your code or upload an .ino file.';
        ...
        $updated = [
            ...
            'code' => $codeContent,
            'author' => $author,
            'date' => $code['date'] ?? date('Y-m-d'),
            'image' => $imagePath
        ];
        
        if (updateAdminCode($id, $updated)) {
            header('Location: ' . BASE_URL . '/admin/?success=updated');
            exit;
        } else {
            $error = 'Failed to save. Check file permissions or database.';
        }
    }
    $code = array_merge($code, $_POST);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Edit Project - <?php echo SITE_NAME; ?> Admin</title>
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-6 bg-white rounded-lg shadow p-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" required
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600"><?php echo htmlspecialchars($code['description']); ?></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="category" name="category" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600">
                        ...
                        <option value="<?php echo $key; ?>" <?php echo $code['category'] === $key ? 'selected' : ''; ?>>
                            <?php echo $cat['name']; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    ...
                    <select id="difficulty" name="difficulty" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600">
                        ...
                        <option value="<?php echo $key; ?>" <?php echo ($code['difficulty'] ?? '') === $key ? 'selected' : ''; ?>>
                            <?php echo $diff['name']; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div>
                ...
                <select id="z2m_part" name="z2m_part" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600">
                    ...
                    <option value="<?php echo htmlspecialchars($partNum); ?>" <?php echo ($code['z2m_part'] ?? '') === $partNum ? 'selected' : ''; ?>>
            ...
            <div>
                ...
                <div class="flex gap-4 mb-3">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" name="code_source" value="paste" id="code_source_paste" checked
                ...
                <div id="code_paste_section" class="code-input-section">
                    <textarea id="code" name="code" rows="20" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 font-mono text-sm"><?php echo htmlspecialchars($code['code']); ?></textarea>
                </div>
            ...
            <div>
                ...
                <p class="text-sm text-gray-600 mb-2">Current: <?php echo htmlspecialchars($code['image']); ?> (upload new to replace)</p>
                <?php endif; ?>
                <input type="file" id="image" name="image" accept="image/jpeg,image/png,image/gif,image/webp"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                <p class="text-xs text-gray-500 mt-1">JPG, PNG, GIF or WebP. Optional.</p>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto border border-gray-200 rounded-lg p-3 bg-gray-50">
                    <?php 
                    $selectedComponents = $code['components'] ?? [];
                    foreach ($component_options as $comp): ?>
                    <label class="inline-flex items-center cursor-pointer hover:bg-gray-100 p-1 rounded">
                        <input type="checkbox" name="components[]" value="<?php echo htmlspecialchars($comp); ?>"
                               ...
                               <?php echo in_array($comp, $selectedComponents) ? 'checked' : ''; ?>>
                ...
                <div class="mt-2">
                    <?php 
                    $selectedComponents = $code['components'] ?? [];
                    $customComps = array_diff($selectedComponents, $component_options);
                    ?>
                    <input type="text" name="components_custom" placeholder="Add custom component (comma-separated)"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm"
                           value="<?php echo htmlspecialchars(implode(', ', $customComps)); ?>">
                </div>
            ...
            <div>
                ...
                <input type="text" id="author" name="author"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600"
                       value="<?php echo htmlspecialchars($code['author'] ?? 'Z2M Codes'); ?>">
            </div>
